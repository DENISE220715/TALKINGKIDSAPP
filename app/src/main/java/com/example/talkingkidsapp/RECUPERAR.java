                ...
                db.collection("users").get().addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
                    ...
                    public void onComplete(@NonNull Task<QuerySnapshot> task) {
                        ...
                        else
                        {
                            ...
                            {
                                if(document.get("correo").toString() == Correo)
                                { Bandera = true; Contraseña = document.get("contraseña").toString(); }
                            }
                            if(!Bandera)
                            { Toast.makeText(RECUPERAR.this, "El correo no fue registrado", Toast.LENGTH_SHORT).show(); }
                            else
                            { vistaContraseña.setText("Tu contraseña es: " + Contraseña); entradaCorreo.setText("");}
                        }
                    }
                });
            }
